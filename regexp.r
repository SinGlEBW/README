...
Примеры
  ...
  a(bc) - ищем "a" с конструкцией "bc". В принципе искать abc это тож самое
...
Жадные выражения ( * )
  (.*at) - The fat cat sat on the mat. - Любые символы, от 0 и больше, заканчиваются слова на at
  Такой подход будет выделит всю строку до последнего слова заканчивающегося на at

Ленивые выражения ( *? )
  (.*?at) - выделит только ту часть строки которая входит до первых встречных букв at
...
переключается на поиск символа >, такого не находит снова возвращается к выполнению условия .+, потом опять проверяет есть ли 
символ > и т.д
-------------------------------------------------------------------------------------------------------------------------------
...
РЕШЕНИЯ НА ПРАКТИКЕ
  ...
  (?<=\/)(\b\w*[-]?\w*\.html)  - найти последее слово в path  
  ^(.*?\s) - берём всё в начале до 1го пробела 
