...
7. sudo npm install pm2 -g  - пакет что бы запускать автоматически сервер если комп перезагрузился у хостинга
                              или не ставить nodejs а использовать docker
8. npm init

  
  Конфиги:
  /etc/apache2/sites-available/   - каталог с настройками для сайтов (как в nginx - /etc/nginx/conf.d/), но созданный конф файл 
                                    нужно включать или выключать и после перезагружать apache2
                                    a2ensite mySite1.conf  - включает конфиг в данном каталоге. a2dissite - выключает
                                    service apache2 restart
                                    



  mariadb-server mariadb-client После установки можно ввести mysql_secure_installation 
  что запустит настройку db. У mysql базы вроде тоже есть такая настройка 

  На mariadb при 1м подключении вылазит ошибка не дающая подключаться это исправляется
  sudo -i
  echo "update user set plugin="where User='root';flush privileges;"| mysql -u root -p mysql"

  Рекомендуют ставить phpMyAdmin с пакетами php-mbstring php-gettext

  wordpress есть в gz архиве, скачать можем через wget или curl в папку нашего сайта /var/www/mySite1
  webmin проверить что за пакет
