...
/*
	Есть одно большое НО. Redux завязан с React таким образом что любое изменение в state вызывает везде mapStateToProps.
	Постоянно возвращается объект. Этот объект на уровне connect и React проверяется. Если он изменился, то будет перерисован render.
	Если использовать "Селекторы" без дополнительного пакета reselect, то при любых изменениях state, будут вызваны mapStateToProps
	которые будут вызывать кучу функций селекторов которые могут не только доставать значения из state и его возвращать
	а делать некие сложные вычисления. 
	К тому же из-за этого не очень удобно дэбажить селекторы т.к. все селекторы отрабатывает от каждого чиха, а не только нужный нам селектор. 

	А самое страшное в данной ситуации это не правильно использовать селекторы и по ошибке возвращать state, тогда при очередном сопоставлении объектов 
	state, функция connect определит эти изменения и будет вызывать постоянно render. Это может произойти из-за использования разных методов типа map, filter, reduce и т.д. 
	все методы которые возвращают новый объект. Происходит постоянный рецикл перерисовки.
	   

*/
//createSelector - 'это функция из пакета reselect 
export const getUsersSelector = createSelector([getUsers], (users) => users.filter(item => item.age > 30) );
/*
	С использованием reselect постоянного вызова render не будет, только по делу. перерисовки не будет
*/