...
//в документации не все свойства указанны т.к. смотреть нужно не MapView, а View
  const view = new eMapView({//настройки показа 2d карты
    ...
    animation,//показывает текущую анимацию установленную через view.goTo()(). Можно использовать метод view.watch() для наблюдения смены анимации
    background: {
...
//Методы
  view.goTo(event.mapPoint);//перемещает карту на заданную координату. Можно организовать событие клика и передавать координату
...
/*#######-------<{ События  View }>---------########### 
  ...
  click, double-click, key-down, mouse-wheel, double-click,
  drag,
...
view.on("key-down", ["Модификатор?"], function(event) {
