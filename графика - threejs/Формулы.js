...
/*
  Синус и косинус принимают длину окружности 
  Длина полного круга в радианах так сложилось исторически это всегда 6.26 или (2*Пи).

  Если представить 2х мерный график "x" и "y" и выкинуть из головы тетрадку в клетку,
  то по сути нет точного понятия расстояния между 0м и 1й по любому из направлений. Измерение
  клетками, миллиметрами, см между этими значениями определяет человек и это лишь шаг.
  Поэтому sin cos выдают диапазон значений от -1 до 1. 

  sin это вертикаль, cos это горизонталь 
  Принимаем длину в радианах на выходе получаем значения по сути катетов.
  console.dir(Math.sin(3.14));//0.0015926529164868282
  console.dir(Math.cos(3.14));//-0.9999987317275395
  Длину от 0 до 6.28 изменяем циклом, как счётчик. Как этот счётчик будет нарастать это уже 
  будет влиять на плавность передвижения гипотенузы.
    
  Что бы построить круг нужно знать в каком месте рисовать точку. Круг рисуется
  прямоугольным треугольником. Точнее его гипотенузой которая перемещается относительно нулевой точки
  и места соприкосновения с окружностью. Можно представить что точку рисуем кончи.
  Длинна гипотенузы это и есть радиус который можно задать.
  
  А дальше самое интересное. Перемещение гипотенузы (то есть угол её наклона) как раз осуществляется изменением катетов,
  размеров катетов. Катеты мы получаем и теперь нужна гипотенуза. Формула

sin()^2 + cos()^2 = гипотенуза^2
   Кто как делает, задача постоянно изменять изменять катеты и постоянно высчитывать гипотенузу
...
/*Гипотенуза. 
  ...
  1е что нуж уяснить что речь действительно идёт о фигуре "Квадрат" и обозначают её письменно Math.pow(10, 2);
     Нужно визуально представлять квадраты на гипотенузе и катетах. (Картинка в этой папке).
  Формула:
*/
с = Math.sqrt(a ** 2 + b ** 2);//c = корень из (a в квадрате + b в квадрате) - находит длину гипотенузы 
...
*/
let deltaX, deltaY;//в событии прокрутки мыши указана скорость колеса прокрутки
