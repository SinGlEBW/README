...
//возможные ограничения
  ...
  callButton.addEventListener('click', () => {
    if(stateMediaTrack){
      ...
      stateMediaTrack = false;
    }
    
  });
  stopCallBtn.addEventListener('click', () => {  (typeof videoCallDisconnect === 'function') && videoCallDisconnect(); })
 

/*------------------------------------------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------------------------------------------
  Вариант без отдельной кнопки звонка. Кто 1й нажат "Старт" тот звонящий.
*/
  ...
  function createConnectP2P(mediaStream) {
    ...
    */
    ...
    createWebSocketConnect(peerConnection, mediaStream);
  }
...
/*------------------------------------------------------------------------------------------------------------------------------------------*/
  ...
  function createWebSocketConnect(peerConnection){
    ...
    socket.addEventListener('open',  (e) => {
      ...
      sendWS({ type: 'initID', clientId, room: selectChanel.value });
      ...
      /*
        Из-за того что на сервере проверяем id и обмениваем данные, то событие message отрабатываем попеременно (один отправил - другой принял).
        Есть вариант отправлять всем без проверки и тогда клиент будет получать так же свой запрос.
      ...
      */
      socket.addEventListener('message', async ({data, ...e}) => {
        ...
        console.dir(data);
        switch (data.type) {
          case 'initID': (data.clientId !== clientId) && sendWS({type: 'peer', caller: clientId, callee: data.clientId }); break;
          case 'peer': (data.caller === clientId) && createOffer(peerConnection, sendWS, data); break;          
          case 'candidate': (data.caller === clientId) ? peerConnection.addIceCandidate(data.candidate) : peerConnection.addIceCandidate(data.candidate); break;      
...
function useAudioContext(mediaStream, options = optionsAudio){
 
  let { filter, gain } = options;
  let audioContext = new AudioContext();
  
  let audioStream = audioContext.createMediaStreamSource(mediaStream);
